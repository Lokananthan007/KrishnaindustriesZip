...
const ProformaConvertInvoice = () => {
  const { follow_id } = useParams();
  const { proforma_Number } = useParams();
  ...
  useEffect(() => {
    if (proforma_Number && invoiceList.length > 0) {
      const selectedData = invoiceList.find((invoice) => invoice.proforma_number === proforma_Number);
      if (selectedData) {
        setSelectedInvoice(proforma_Number); 
        handleInvoiceChange({ target: { value: proforma_Number } });
      }
    }
  }, [proforma_Number, invoiceList]);


  const fetchDefaultInvoiceData = () => {
    if (toDetails.leads_id) {
      axios
        .get(`${config.apiUrl}/quotation/proforma/leads/${toDetails.leads_id}`)
        .then((res) => {
  ...
  const handleInvoiceChange = (event) => {
    ...
    if (value === '') {
      ...
      const selectedData = invoiceList.find((invoice) => invoice.proforma_number === value);

      if (selectedData) {
        setProducts(selectedData.product_details);
        setInvoiceDate(selectedData.proforma_data);
        setInvoiceNumber(selectedData.proforma_number);
            ...
            <Grid item xs={6} sm={3} margin={'13px'}>
              ...
              >
                ...
                {invoiceList.map((invoice) => (
                  <MenuItem key={invoice.id} value={invoice.proforma_number}>
...
export default ProformaConvertInvoice;
